...
interface Props {
  ...
  /** Omit GPX `<wpt>` points from the map (track line only). */
  gpxLineOnly?: boolean
  /** `terminals` = first/last waypoint by `order` (one pin if same spot). */
  ...
  if (!props.route.mapCenter) {
    return (
      <div className="planning-map planning-map--empty" role="status">
        No map — there’s no centre in the data for this page.
      </div>
    )
  }
...
function RouteMapOpenStreetMap({
  route,
  waypoints,
  selectedOptionId,
  gpxLineOnly = false,
